...
package com.github.ImFlog.avro.message;
...
import com.github.ImFlog.avro.generic.GenericRecordBuilder;
...
public class TestBinaryMessageEncoding {
  ...
  private static final List<GenericData.Record> V2_RECORDS = Arrays.asList(
  ...
  void byteBufferRoundTrip() throws Exception {
    MessageEncoder<GenericData.Record> encoder = new BinaryMessageEncoder<>(GenericData.get(), SCHEMA_V2);
    MessageDecoder<GenericData.Record> decoder = new BinaryMessageDecoder<>(GenericData.get(), SCHEMA_V2);

    GenericData.Record copy = decoder.decode(encoder.encode(V2_RECORDS.get(0)));
  ...
  void schemaEvolution() throws Exception {
    ...
    MessageEncoder<GenericData.Record> v1Encoder = new BinaryMessageEncoder<>(GenericData.get(), SCHEMA_V1);
    MessageEncoder<GenericData.Record> v2Encoder = new BinaryMessageEncoder<>(GenericData.get(), SCHEMA_V2);
    ...
    Set<GenericData.Record> allAsV2 = new HashSet<>(V2_RECORDS);
    ...
    Set<GenericData.Record> decodedUsingV2 = new HashSet<>();
  ...
  void compatibleReadWithSchema() throws Exception {
    ...
    GenericData.Record record = v2Decoder.decode(v1Buffer);

    Assertions.assertEquals(V2_BUILDER.set("id", 6L).set("message", "m-6").clear("data").build(), record);
  }
  ...
  void compatibleReadWithSchemaFromLookup() throws Exception {
    ...
    Assertions.assertEquals(V2_BUILDER.set("id", 4L).set("message", "m-4").clear("data").build(), record);
  }
  ...
  void identicalReadWithSchemaFromLookup() throws Exception {
    ...
    GenericData.Record record = genericDecoder.decode(v1Buffer);
  ...
  void bufferReuse() throws Exception {
    ...
    MessageEncoder<GenericData.Record> encoder = new BinaryMessageEncoder<>(GenericData.get(), SCHEMA_V1, false);
    ...
    MessageDecoder<GenericData.Record> decoder = new BinaryMessageDecoder<>(GenericData.get(), SCHEMA_V1);
  ...
  void bufferCopy() throws Exception {
    MessageEncoder<GenericData.Record> encoder = new BinaryMessageEncoder<>(GenericData.get(), SCHEMA_V1);
