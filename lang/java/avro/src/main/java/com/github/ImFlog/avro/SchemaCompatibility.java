    ...
    private SchemaCompatibilityResult calculateCompatibility(final Schema reader, final Schema writer,
      ...
      if (reader.getType() == writer.getType()) {
        ...
        case INT:
          return result.mergedWith(typeMismatch(reader, writer, location));
        case LONG: {
          return (writer.getType() == Schema.Type.INT) ? result
              : result.mergedWith(typeMismatch(reader, writer, location));
        }
        case FLOAT: {
          return ((writer.getType() == Schema.Type.INT) || (writer.getType() == Schema.Type.LONG)) ? result
              : result.mergedWith(typeMismatch(reader, writer, location));

        }
        case DOUBLE: {
          return ((writer.getType() == Schema.Type.INT) || (writer.getType() == Schema.Type.LONG)
              || (writer.getType() == Schema.Type.FLOAT)) ? result
                  : result.mergedWith(typeMismatch(reader, writer, location));
        }
        case BYTES: {
          return (writer.getType() == Schema.Type.STRING) ? result
              : result.mergedWith(typeMismatch(reader, writer, location));
        }
        case STRING: {
          return (writer.getType() == Schema.Type.BYTES) ? result
              : result.mergedWith(typeMismatch(reader, writer, location));
        }
